...
- [ ] ILM (Index Lifecycle Management) для Elasticsearch: реализовать политику удаления старых данных (StackConfigPolicy, ECK 3.3+). Требуется Enterprise-лицензия ECK или trial. Нужно не только создать политику, но и привязать её к индексам nodestore (index template, `index.lifecycle.name` или `PUT` настроек индекса) — само наличие ILM в кластере не меняет существующие индексы. Манифест `index-lifecycle-policy-delete.yaml` был удалён из репозитория до полной проработки.
