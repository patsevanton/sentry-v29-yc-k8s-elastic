...
1. Создайте токен через [Internal Integration](https://docs.sentry.io/product/integrations/integration-platform/internal-integration/). В UI Sentry 26+: **Settings** → в левой колонке **Developer Settings** → **Custom Integrations** → **Create New Integration** → тип **Internal Integration** → **Next** → имя интеграции, например **`vm-sentry-prometheus-exporter`**. В блоке **Permissions** для [sentry-prometheus-exporter](https://github.com/italux/sentry-prometheus-exporter) нужны те же возможности, что дают scope в API: `org:read`, `project:read`, `project:releases`, `event:read` ([описание scope](https://docs.sentry.io/api/permissions/)). В интерфейсе это не отдельные строки `org:read`, а выпадающие списки по категориям — выставьте минимум **Read** там, где это соответствует чтению организации, проектов, релизов и событий: **Organization** — **Read**, **Project** — **Read**, **Release** — **Read**, **Issue & Event** — **Read**; остальные категории (**Team**, **Distribution**, **Member**, **Alerts** и т.д.) оставьте **No Access**, если они не нужны. Полный набор из upstream; при отключённых метриках по событиям/релизам иногда хватает части прав, проще включить **Read** по всем четырём категориям выше. **Save** → внизу страницы скопируйте **Token** (на этой же странице можно создать до 20 токенов на интеграцию). См. [как создать auth token](https://docs.sentry.io/api/guides/create-auth-token/) и [документацию API](https://docs.sentry.io/api/auth/).
...
kubectl create secret generic sentry-dsn-node -n demo-sentry \
  --from-literal=dsn='http://dd023a73da57de4d180bbfa3b3318633@sentry.apatsev.org.ru/2'
kubectl create secret generic sentry-dsn-python -n demo-sentry \
  --from-literal=dsn='http://ea803175116dbc3cd8734c8f14522956@sentry.apatsev.org.ru/3'
